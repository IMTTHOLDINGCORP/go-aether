...
import (
	...
	"github.com/IMTTHOLDINGCORP/go-aether/event"
	"github.com/IMTTHOLDINGCORP/go-aether/log"
	"github.com/IMTTHOLDINGCORP/go-aether/metrics"
	...
	for _, kv := range pool.pending.asList() {
		...
		if len(txs) > 0 {
			if tx := txs[len(txs)-1]; tx != nil {
				...
				log.Error("IMTTHOLDINGCORP_TODO_001 : 1", "addr", addr.Hex(), "len", len(txs))
			}
		} else {
			log.Error("IMTTHOLDINGCORP_TODO_001 : 2", "addr", addr.Hex(), "len", len(txs))
		}
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	if err := pool.validateRegister(tx); err != nil {
		return err
	}
...
/*
{
  ...
  "Name":"资源名称",
  "Desc":"概述，选填",
  "Label":"标签，用逗号隔开，例如 '武侠,科幻,犯罪' 等",
  "Type":"资源分类,暂时分 视频和音频两类吧"
}
*/
type IndexVo struct {
	Id, Img, Name, Desc, Label, Type string
}

// add by liangc
func (pool *TxPool) validateRegister(tx *types.Transaction) error {
	if tx.To() != nil &&
		pool.chainconfig.Register001Address != common.HexToAddress("") &&
		pool.chainconfig.Register001Block != big.NewInt(0) &&
		pool.chainconfig.Register001Address == *tx.To() {
		mid := tx.Data()[:4]
		a, err := abi.JSON(strings.NewReader(params.Register_0_0_1ABI))
		if err != nil {
			return err
		}
		m := a.MethodById(mid)
		if m == nil {
			return errors.New("error_method_of_register")
		} else if m.Name == "appendId" {
			// format validator
			data := tx.Data()[4:]
			id := ""
			rawdata := ""
			...
			log.Info("1 --reg_tx-->", "rawdata", rawdata)
			iv := new(IndexVo)
			err = json.Unmarshal([]byte(rawdata), iv)
			if err != nil {
				return err
			}
			log.Info("2 --reg_tx-->", "idxvo", iv)
			if iv.Id != id || len(iv.Id) != 46 {
				return errors.New("appendId_error_input_id")
			}
			if iv.Img != "" && len(iv.Img) != 46 {
				return errors.New("appendId_error_input_img")
			}
			switch "" {
			case iv.Id, iv.Name, iv.Label, iv.Type:
				return errors.New("id_name_label_type__cannot_empty")
			}
		}
	}
	return nil
...
func (pool *TxPool) demoteUnexecutables() {
	...
	for _, kv := range pool.pending.asList() {
		...
		// add by liangc
		if list == nil {
			log.Error("IMTTHOLDINGCORP_TODO_002", "addr", addr.Hex(), "list", list)
			continue
		}
		if list.txs == nil {
			log.Error("IMTTHOLDINGCORP_TODO_002", "addr", addr.Hex(), "list.txs", list.txs)
			continue
		}
