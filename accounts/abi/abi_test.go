...
func TestRegisterContract(t *testing.T) {
	...
	t.Log(Register_0_0_1ABI)
	hexdata := ""
	data, err := hex.DecodeString(hexdata)
	t.Log("1 err=", err, data)
	abi, err := JSON(strings.NewReader(Register_0_0_1ABI))
	t.Log("2 err=", err)
	method := abi.Methods["appendId"]
	id := ""
	rawdata := ""
	r := []interface{}{&id, &rawdata}
	err = method.Inputs.Unpack(&r, data[4:])
	t.Log("4 err=", err, r, id, rawdata)

}

func TestTribemapContract(t *testing.T) {
	...
	hexdata := "9dfad1e80000000000000000000000004fc86e2abb09ba52944d040fd07e200615a200800000000000000000000000000000000000000000000000000000000000002710"
	data, err := hex.DecodeString(hexdata)
	t.Log("1 err=", err, data)
	abi, err := JSON(strings.NewReader(TribemapABI))
	method := abi.Methods["makeCoin"]
	t.Log("2 err=", err, method.Inputs)
	...
	err = method.Inputs.Unpack(&r, data[4:])
	t.Log("4 err=", err, addr.Hex(), val.Int64())
