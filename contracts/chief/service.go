...
import (
	...
	"github.com/IMTTHOLDINGCORP/go-aether/contracts/chief/lib"
			...
			if len(vlist) > 0 {
				...
				if rlist, err := self.tribeChief_0_0_6.FilterVolunteer(opts, vlist); err == nil {
					...
					for i, r := range rlist {
						if r.Int64() > 0 {
							v := vlist[i]
							if ts, err := tribemap.GetTribemapService(); err == nil {
								log.Info("<< fetchVolunteer.tribemap-rule >>")
								if w, err := ts.ExistAddress(v); err == nil && w != nil && w.Int64() > 0 {
									return v
								}
								log.Info("<< fetchVolunteer.skip >> not_a_tribemap", "addr", v.Hex())
							} else {
