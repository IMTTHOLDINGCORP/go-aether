...
import (
	...
	"github.com/IMTTHOLDINGCORP/go-aether/consensus/tribe"
...
func (self *worker) start(s chan int) {
	...
	if tribe, ok := self.engine.(*tribe.Tribe); ok {
		if self.chain.CurrentBlock().NumberU64() > 2 {
			wg.Add(1)
			go func() {
				defer wg.Done()
	...
	for {
		...
		case ev := <-self.txCh:
			...
			if atomic.LoadInt32(&self.mining) == 0 {
				...
				if self.config.Tribe != nil && self.config.Tribe.Period == 0 {
					if tribe, ok := self.engine.(*tribe.Tribe); ok && tribe.WaitNewWork() {
						//TODO 收到新交易就去打包块
						log.Info(" -- 📦 --> todo call commitNewWork() ... ")
						self.commitNewWork()
					}
				}
			}
			...
			if result == nil {
				...
				if tribe, ok := self.engine.(*tribe.Tribe); ok {
					...
					if sealErr, ok := tribe.SealErrorCh[h.Number.Int64()]; ok {
						log.Debug("SealErrorCh", "currentNumber", h.Number.Int64(), "err", sealErr)
						log.Debug("wait_new_work_will_retry_tribe.update", "current_num", h.Number.Int64(), "current_hash", h.Hash().Hex())
						tribe.Status.Update(h.Number, h.Hash())
						self.commitNewWork()
						log.Debug("wait_new_work_will_retry_commitNewWork", "current_num", h.Number.Int64(), "current_hash", h.Hash().Hex())
	...
	for _, _v := range pending {
		for _, _tx := range _v {
			if _tx.To() != nil && params.IsChiefAddress(*_tx.To()) {
				log.Debug("-chiefTx->", "cheifTx", _tx.Hash().Hex(), "input", _tx.Data())
			}
		}
	}
	//debug <--- */
...
func (env *Work) commitTransactions(mux *event.TypeMux, txs *types.TransactionsByPriceAndNonce, bc *core.BlockChain, coinbase common.Address) {
	...
	for {
		...
		if err != nil {
			if tx.To() != nil {
				log.Info("IMTTHOLDINGCORP_TODO_004", "cn", bc.CurrentHeader().Number.Int64(), "tx", tx.Hash().Hex(), "to", *tx.To(), "err", err)
			} else {
				log.Info("IMTTHOLDINGCORP_TODO_004", "cn", bc.CurrentHeader().Number.Int64(), "tx", tx.Hash().Hex(), "err", err)
			}
