...
func New(eth Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	...
	go miner.update()
	if _tribe, ok := miner.engine.(*tribe.Tribe); ok {
		close(params.TribeReadyForAcceptTxs)
		log.Info("params.TribeReadyForAcceptTxs")
		go func() {
			rtn := make(chan common.Address)
			_tribe.Status.GetMinerAddressByChan(rtn)
			tma := <-rtn
			_tribe.Authorize(common.Address{}, nil)
			go miner.Start(tma)
			log.Info("👷 Tribe and miner is started .")
			/*
			switch _tribe.Status.SignerLevel {
			case tribe.LevelVolunteer, tribe.LevelSigner:
				go miner.Start(tma)
				log.Info("👷 Tribe and miner is started .")
			}*/
		}()
