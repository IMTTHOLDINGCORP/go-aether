...
import (
	...
	"github.com/IMTTHOLDINGCORP/go-aether/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of geth is "aether". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "aether", keep that.
	if name == "aether" || name == "aether-testnet" {
		name = "aether"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "aether" instances.
...
func (c *Config) resolvePath(path string) string {
	...
	if c.name() == "aether" && isOldGethResource[path] {
		oldpath := ""
		if c.Name == "aether" {
...
func (c *Config) loadECDSAWithPwd() (*ecdsa.PrivateKey, error) {
	...
	overFunc := func(){
		fmt.Println("=================================================")
		fmt.Println(`> Must be first execute "aether security --unlock" `)
		fmt.Println("=================================================")
		os.Exit(-1)
	}
